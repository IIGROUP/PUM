...
from config import VG_IMAGES, IM_DATA_FN, VG_SGG_FN, VG_SGG_DICT_FN, BOX_SCALE, IM_SCALE, PROPOSAL_FN, cfg
...
class VG(Dataset):
    def __init__(self, mode, roidb_file=VG_SGG_FN, dict_file=VG_SGG_DICT_FN,
        ...
        if cfg.obj_dists_path != '' and self.mode != 'train':
            obj_dists_path = cfg.obj_dists_path
            if self.mode == 'test':
                obj_dists_path = obj_dists_path.replace('train', 'test')
            # Read cached results
            self.obj_dists = pickle.load(open(obj_dists_path, 'rb'))
            print('Load cached object distribution results from %s' % obj_dists_path)
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, 'obj_dists'):
            entry['obj_dists'] = self.obj_dists[index]

        if cfg.obj_det_path != '' and (self.mode == 'test' or cfg.fixed_obj_det_in_training):
            obj_det_dir = cfg.obj_det_path
            assert os.path.isdir(obj_det_dir), 'Directory of obj det cache expected'
            if self.mode == 'val':
                obj_det_dir = obj_det_dir.replace('train', 'val')
            elif self.mode == 'test':
                obj_det_dir = obj_det_dir.replace('train', 'test')
            img_id = os.path.basename(entry['fn']).split('.')[0]
            cache_path = os.path.join(obj_det_dir, '%s.pkl' % img_id)
            entry['obj_det'] = pickle.load(open(cache_path, 'rb'))
