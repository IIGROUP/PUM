...
from lib.pytorch_misc import arange, sample_z, get_mean_dists_z
...
class IMP(Motifs):
    ...
    def __init__(self, classes, rel_classes, mode='sgdet', num_gpus=1, require_overlap_det=True,
        ...
        """
        super(IMP, self).__init__(classes, rel_classes, mode=mode, num_gpus=num_gpus,
        ...
        if cfg.visual_gaussian:
            self.gaussian_emb = nn.Linear(SIZE, SIZE * 2)
    ...
    def message_pass(self, rel_rep, obj_rep, rel_inds):
        ...
        if cfg.visual_gaussian:
            gaussian = self.gaussian_emb(edge_factor[-1])
            self.mu, self.log_var = torch.split(gaussian, SIZE, dim=1)
            z = sample_z(self.mu, self.log_var, cfg.num_gaussian_samples)
            rel_dists = self.rel_fc(self.mu)
            rel_dists_z = self.rel_fc(z)
            return obj_dists, rel_dists, rel_dists_z
        else:
            return obj_dists, self.rel_fc(edge_factor[-1])
