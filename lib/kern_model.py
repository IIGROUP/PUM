...
class GGNNRelReason(nn.Module):
    ...
    def forward(self, obj_fmaps, obj_logits, rel_inds, vr, obj_labels=None, boxes_per_cls=None):
        ...
        if self.training:
            ggnn_rel_out = self.ggnn_rel(rel_inds[:, 1:], sub_obj_preds, input_ggnn)
        else:
            # Split the inputs into multiple pieces and feed more than once to save memory
            ggnn_rel_out = []
            split_size = cfg.test_split_size
            num_groups = int(math.ceil(input_ggnn.shape[0] / split_size))
            for i in range(num_groups):
                end = int((i + 1) * split_size)
                rel_inds_i = rel_inds[:, 1:][i * split_size: end]
                sub_obj_preds_i = sub_obj_preds[i * split_size: end]
                input_ggnn_i = input_ggnn[i * split_size: end]
                ggnn_rel_out_i = self.ggnn_rel(rel_inds_i, sub_obj_preds_i, input_ggnn_i)
                ggnn_rel_out.append(ggnn_rel_out_i)
            ggnn_rel_out = torch.cat(ggnn_rel_out)

        if cfg.visual_gaussian and self.training:
            rel_dists, rel_dists_z = ggnn_rel_out
            return obj_dists2, obj_preds, rel_dists, rel_dists_z
        else:
            rel_dists = ggnn_rel_out
...
class KERN(nn.Module):
    ...
    def regularization_term(self):
        assert cfg.visual_gaussian
        return self.ggnn_rel_reason.ggnn_rel.regularization_term()
    ...
    def forward(self, x, im_sizes, image_offset,
        ...
        if self.use_ggnn_rel:
            ggnn_rel_reason_out = self.ggnn_rel_reason(
            ...
            if cfg.visual_gaussian and self.training:
                result.rm_obj_dists, result.obj_preds, result.rel_dists, result.rel_dists_z = ggnn_rel_reason_out
            else:
                result.rm_obj_dists, result.obj_preds, result.rel_dists = ggnn_rel_reason_out
        else:
