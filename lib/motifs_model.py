...
from config import BATCHNORM_MOMENTUM, cfg, MODES
...
from lib.pytorch_misc import transpose_packed_sequence_inds, to_onehot, arange, enumerate_by_image, diagonal_inds, Flattener, \
    sample_z, get_mean_dists_z, gaussian_regularization, nms_post_processing
...
def _sort_by_score(im_inds, scores):
    ...
    """
    num_im = im_inds[-1] + 1
    # rois_per_image = scores.new(num_im)  # Not work with Pytorch 0.4
    rois_per_image = scores.new(int(num_im))
    lengths = []
...
class Motifs(nn.Module):
    ...
    def forward(self, x, im_sizes, image_offset,
                ...
                return_fmap=False, obj_dists=None, det_results_cache=None):
        """
        ...
        """
        if det_results_cache:
            result = det_results_cache
        else:
        ...
        res = filter_dets(bboxes, result.obj_scores,
