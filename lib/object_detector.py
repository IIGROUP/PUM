...
class Result(object):
    ...
    def __init__(self, od_obj_dists=None, rm_obj_dists=None,
                 ...
                 sbj_dists=None, obj_dists=None, rel_dists_z=None):
...
class ObjectDetector(nn.Module):
    ...
    def forward(self, x, im_sizes, image_offset,
        ...
        # For each class, produce box delta independently
    ...
    def nms_boxes(self, obj_dists, rois, box_deltas, im_sizes):
        ...
        """
        # Now produce the boxes, based on the rois from RPN and the corresponding box delta regressions
        ...
        dets = []
        for i, s, e in enumerate_by_image(inds.data):  # Process the images one by one
            h, w = im_sizes[i, :2]
            ...
            d_filtered = filter_det(
                ...
                thresh=self.thresh,  # Threshold to filter out low-score boxes
            )
        ...
        # nms_inds: indexes of #boxes
        # nms_labels: indexes of all 151 class categories
        twod_inds = nms_inds * boxes.size(1) + nms_labels.data  # To index flattened boxes (shape of [#boxes*151, 4])
        nms_boxes_assign = boxes.view(-1, 4)[twod_inds]

        # `rois[:, 1:][nms_inds][:, None]` (shape of [64, 1, 4]) provides the original coordinates from RPN (or box priors)
        # `boxes[nms_inds][:, 1:]` (shape of [64, 150, 4]) provides coordinates for the 150 classes
...
def filter_det(scores, boxes, start_ind=0, max_per_img=100, thresh=0.001, pre_nms_topn=6000,
    ...
    for c_i in valid_cls.squeeze(1).cpu():
        ...
        # For each valid class, assume all boxes belong to this class and apply NMS
    ...
    with torch.no_grad():
        dists_all = Variable(nms_mask * scores.data)  # Set scores of those filtered out to 0

    if nms_filter_duplicates:
        # For each box, only take the arg-max class and filter out boxes that never act as kept in NMS for every class
