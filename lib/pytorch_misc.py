...
def transpose_packed_sequence_inds(lengths):
    ...
    new_lens = torch.IntTensor(new_lens)
...
def sample_z(mu, log_var, n):
    """
    Sample z from a batch of Gaussians
    :return: (batch_size, N, feat_dim)
    """
    eps = torch.randn(mu.shape[0], n, mu.shape[-1], device=mu.device)
    z = mu.unsqueeze(1) + torch.exp(log_var.unsqueeze(1) / 2) * eps
    z = z.view([z.shape[0] * z.shape[1], -1])  # (B*N, feat_dim)
    return z


def get_mean_dists_z(rel_dists_z, batch_size, cfg):
    mean_dists_z = torch.mean(rel_dists_z.view([batch_size, cfg.num_gaussian_samples, -1]), 1)
    return mean_dists_z


def gaussian_regularization(mu, log_var, cfg):
    feat_dim = log_var.shape[-1]
    entropy = float(feat_dim / 2 * (np.log(2 * np.pi) + 1)) + torch.sum(log_var, 1) / 2
    entropy = torch.mean(entropy)
    if cfg.gaussian_reg == 'entropy':
        zero = torch.zeros_like(entropy)
        reg_term = torch.max(cfg.uncer_margin - entropy, zero)
    elif cfg.gaussian_reg == 'vib':
        kl = 1 / 2 * torch.sum(torch.exp(log_var) + mu ** 2 - log_var - 1, dim=1)
        reg_term = torch.mean(kl)
    else:
        reg_term = torch.zeros([], device=log_var.device)
    reg_term *= cfg.reg_weight
    return reg_term, entropy


def gather_gaussian(model, replicas, target_device=0):
    """Gather mu and logvar in case of multi-GPU training"""
    model.mu = Gather.apply(target_device, 0, *[replica.mu for replica in replicas])
    model.log_var = Gather.apply(target_device, 0, *[replica.log_var for replica in replicas])


def nms_post_processing(boxes_for_nms, obj_dists, nms_thresh=0.3):
    """NMS post-processing, refer to Motifs"""
    ...
    out_dists_sampled = F.softmax(obj_dists, 1).data.cpu().numpy()
    out_dists_sampled[:, 0] = 0

    out_commitments = torch.zeros([obj_dists.shape[0]], device=obj_dists.device).long()
...
def nps_loss(inputs, gt_classes, gt_rels, im_inds, use_weight=False):
    ...
    for i in range(img_nums):
        mask = im_inds == i
        count_sum = (pairs_count[mask.data]).sum()
        factor = 1.0 / count_sum if count_sum else 0  # consider situation where count is 0
        pairs_count[mask.data] *= factor

    if not use_weight:
        # bounding = torch.zeros_like(pairs_count).fill_(2.0)
        # gama_powers = torch.min(, bounding)
        gama_powers = torch.clamp(-((1 - 2.0*pairs_count) ** 5.0) * torch.log(2.0*pairs_count.clamp(min=1e-8)), max=2.0)
...
def focal_loss(pred, target, gamma=2.0, alpha=None, size_average=True):
    if pred.dim()>2:
